...
async def get_dp(
    ...
    limit = f' limit {limit}'
    data = {
        "dp": [{'id': user.dp, 'name': user.dpName}] if user.role < 5 else await db_pools.mysql.fetch_all(f"select * from dic_department{limit}"),
        "group": await db_pools.mysql.fetch_all("select * from dic_group")
    }
    if total:
        data['total'] = 1 if user.role < 5 else (await db_pools.mysql.fetch_one("select count(*) c from dic_department"))['c']
    return {'detail': data}
...
async def post_dp(
    user: User = role_verify(2),
    name: str = Body(...)):
    return {'detail': await db_pools.mysql.execute(f"insert into dic_department(name,uid) values ('{name}',{user.uid}")}
...
async def put_dp(
    ...
    dpid: str = Body(...),
    name: str = Body(...)):
    return {'detail': await db_pools.mysql.execute(f"update dic_department set name='{name}',uid={user.uid} where id='{dpid}'")}
